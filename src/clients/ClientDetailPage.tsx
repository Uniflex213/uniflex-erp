...
export default function ClientDetailPage({
  ...
  const TABS: { key: Tab; label: string; count?: number }[] = [
    ...
    { key: "orders", label: `Commandes (${clientOrders.length})` },
    { key: "pricelists", label: `Soumissions (${pricelists.filter(p => p.companyName === client.company_name).length})` },
    { key: "samples", label: `Samples (${clientSamples.length})` },
    { key: "notes", label: `Notes (${clientNotes.length})` },
    { key: "credit_notes", label: `Notes crédit (${creditNotes.length})` },
    { key: "disputes", label: `Disputes (${disputes.filter(d => d.status === "Ouverte" || d.status === "En cours").length > 0 ? `🔴 ${disputes.length}` : disputes.length})` },
    { key: "pickup", label: `Pickup (${realPickupTickets.length})` },
    ...(client.is_converted_lead ? [{ key: "crm_history" as Tab, label: "Historique CRM" }] : []),
    { key: "change_log" as Tab, label: "Modifications" },
  ];
    ...
    <div style={{ fontFamily: "'Outfit', sans-serif", color: T.text, minHeight: "100%" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
              ...
              {isInactive && <span style={{ fontSize: 11, fontWeight: 700, background: "rgba(245,158,11,0.15)", color: T.orange, padding: "3px 9px", borderRadius: 10 }}>Inactif</span>}
            </div>
            ...
            <div style={{ fontSize: 13, color: T.textMid }}>
              Agent : <strong style={{ color: T.text }}>{client.agent_name}</strong>
            </div>
          </div>

          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <ActionButton label="📦 Nouvelle commande" color={T.blue} onClick={() => navigate?.("orders", { companyName: client.company_name, address: billingAddr, contactName: `${client.contact_first_name} ${client.contact_last_name}`, clientEmail: client.email, clientPhone: client.phone, clientId: client.id })} />
            <ActionButton label="📄 Nouvelle pricelist" color={T.main} onClick={() => navigate?.("pricelist", { companyName: client.company_name, address: billingAddr, contactName: `${client.contact_first_name} ${client.contact_last_name}`, clientEmail: client.email, clientPhone: client.phone, clientId: client.id })} />
            <ActionButton label="📋 Note de crédit" color={T.orange} onClick={() => { setTab("credit_notes"); setShowCreditForm(true); }} />
            <ActionButton label="⚠️ Dispute" color={T.red} onClick={() => { setTab("disputes"); setShowDisputeForm(true); }} />
            <ActionButton label="📊 Rapport PDF" color="#059669" onClick={() => generateClientPDF(client, clientNotes, creditNotes, disputes)} />
            <ActionButton label="✏️ Modifier" color={T.textMid} onClick={() => setShowEdit(true)} />
            {isAdmin && onReassignClient && (
              <ActionButton label="Réassigner" color="#0891b2" onClick={() => onReassignClient(client)} />
            )}
            {isAdmin && onDeleteClient && (
              <ActionButton label="Supprimer" color={T.red} onClick={() => onDeleteClient(client.id)} />
            )}
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: 24 }}>
        {tab === "info" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, maxWidth: 900 }}>
            <div style={{ background: T.card, borderRadius: 12, padding: 20, border: `1px solid ${T.border}` }}>
              <div style={{ fontWeight: 800, fontSize: 13, marginBottom: 14, color: T.text }}>Contact & Coordonnées</div>
              {[
                ["Compagnie", client.company_name],
                ["Contact", `${client.contact_first_name} ${client.contact_last_name}`],
                ["Poste", client.contact_title || "—"],
                ["Téléphone", client.phone],
                ["Téléphone 2", client.phone_secondary || "—"],
                ["Email", client.email],
                ["Site web", client.website || "—"],
                ["Facturation", billingAddr || "—"],
                ["Livraison", shippingAddr || "—"],
              ].map(([k, v]) => (
            ...
            <div style={{ background: T.card, borderRadius: 12, padding: 20, border: `1px solid ${T.border}` }}>
              <div style={{ fontWeight: 800, fontSize: 13, marginBottom: 14, color: T.text }}>Classification & Commercial</div>
              {[
                ["Type", client.client_type],
                ["Tier", client.tier],
                ["Région", client.region || "—"],
                ["Code client", client.client_code],
                ["Paiement", client.payment_terms],
                ["Devise", client.currency],
                ["Commission", client.special_commission_rate != null ? `${client.special_commission_rate}%` : "Standard"],
                ["Source", client.source],
                ["Depuis le", fmtDate(client.created_at)],
              ].map(([k, v]) => (
                <div key={k} style={{ display: "flex", gap: 10, marginBottom: 8 }}>
                  <span style={{ fontSize: 12, color: T.textLight, width: 90, flexShrink: 0, fontWeight: 600 }}>{k}</span>
                  <span style={{ fontSize: 12, color: T.text }}>{v}</span>
              ...
              <div style={{ display: "flex", gap: 10, marginBottom: 8 }}>
                <span style={{ fontSize: 12, color: T.textLight, width: 90, flexShrink: 0, fontWeight: 600 }}>Liste prix</span>
                {client.pricelist_pdf_url ? (
                  ...
                  <span style={{ fontSize: 12, color: T.textLight }}>Aucune</span>
                )}
              </div>
            </div>
          </div>
        ...
        {tab === "orders" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>Commandes ({clientOrders.length})</h3>
            ...
            {ordersLoading ? (
              ...
              <div style={{ textAlign: "center", padding: "40px 20px", color: T.textLight, fontSize: 14 }}>Aucune commande pour ce client.</div>
            ) : (
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: T.bg }}>
                    {["# Commande", "Date", "Produits", "Montant", "Statut", "Facturation"].map(h => (
        ...
        {tab === "pricelists" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>Soumissions / Pricelists</h3>
            ...
            {pricelists.filter(p => p.companyName === client.company_name).length === 0 ? (
              <div style={{ textAlign: "center", padding: "40px 20px", color: T.textLight, fontSize: 14 }}>Aucune pricelist générée pour ce client.</div>
            ) : (
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: T.bg }}>
                    {["ID", "Date", "Devise", "Lignes", "Validité"].map(h => (
                      <th key={h} style={{ padding: "10px 12px", textAlign: "left", fontWeight: 700, fontSize: 11, color: T.textMid, textTransform: "uppercase", letterSpacing: 0.4 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {pricelists.filter(p => p.companyName === client.company_name).map(pl => (
                    <tr key={pl.id} style={{ borderBottom: `1px solid ${T.border}` }}>
                      ...
                      <td style={{ padding: "11px 12px" }}>{pl.lines.length} produits</td>
      ...
      <div style={{ padding: "0 24px 32px" }}>
        <div style={{ background: T.card, borderRadius: 12, padding: 20, border: `1px solid ${T.border}`, maxWidth: 700 }}>
          <div style={{ fontWeight: 800, fontSize: 13, marginBottom: 12, color: T.text }}>Notes rapides (auto-save)</div>
          <textarea
            value={notes}
            ...
            placeholder="Notes rapides sur ce client..."
          />
        </div>
      </div>
