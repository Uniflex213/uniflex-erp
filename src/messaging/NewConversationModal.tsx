...
export default function NewConversationModal({ currentUser, allowedRoles, onStart, onClose }: Props) {
  ...
  const tabs: { key: Tab; label: string; show: boolean }[] = [
    { key: "team", label: t("msg.my_team"), show: !!currentUser.team_id },
    { key: "search", label: t("msg.search_tab"), show: true },
    { key: "contacts", label: t("msg.contacts_tab"), show: true },
  ];
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: T.text, display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {isSameTeam && (
              <span style={{ fontSize: 9, fontWeight: 700, background: T.teamBg, color: T.main, padding: "1px 6px", borderRadius: 4, flexShrink: 0 }}>{t("msg.team_badge")}</span>
            )}
          </div>
    ...
    <div style={{ position: "fixed", inset: 0, background: T.overlay, backdropFilter: "blur(4px)", zIndex: 9700, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: T.card, borderRadius: 16, width: "100%", maxWidth: 440, maxHeight: "82vh", display: "flex", flexDirection: "column", boxShadow: "0 32px 80px rgba(0,0,0,0.5)", border: `1px solid ${T.border}` }}>
        <div style={{ padding: "16px 18px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontSize: 15, fontWeight: 800, color: T.text }}>{t("msg.new_conversation")}</span>
          ...
          {tabs.filter(tb => tb.show).map(tb => (
            <button
              ...
              style={{
                ...
                borderBottom: tab === tb.key ? `2px solid ${T.main}` : "2px solid transparent",
                cursor: "pointer", padding: "10px 14px", fontSize: 12,
                fontWeight: tab === tb.key ? 700 : 500,
                color: tab === tb.key ? T.text : T.textMid,
        ...
        {tab === "team" && (
          <>
            ...
            <div style={{ flex: 1, overflowY: "auto" }}>
              {loadingTeam ? (
                <div style={{ padding: 24, textAlign: "center", color: T.textMid, fontSize: 13 }}>{t("loading_dots")}</div>
              ) : filteredTeammates.length === 0 ? (
                <div style={{ padding: "32px 16px", textAlign: "center", color: T.textMid, fontSize: 13, lineHeight: 1.6 }}>
                  {teammates.length === 0 ? t("msg.no_teammates") : t("msg.no_filter_results")}
                </div>
              ) : (
        ...
        {tab === "search" && (
          <>
            <div style={{ padding: "14px 14px 10px", borderBottom: `1px solid ${T.border}` }}>
              <div style={{ fontSize: 12, color: T.textMid, marginBottom: 8 }}>{t("msg.search_by_email")}</div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, background: T.input, borderRadius: 8, padding: "8px 12px" }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={T.textMid} strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                <input
                  value={emailQuery}
                ...
                >
                  {searching ? "..." : t("msg.search_btn")}
                </button>
              </div>
            </div>
            <div style={{ flex: 1, overflowY: "auto" }}>
              {searching ? (
                <div style={{ padding: 24, textAlign: "center", color: T.textMid, fontSize: 13 }}>{t("msg.searching")}</div>
              ) : searchDone && searchResults.length === 0 ? (
                <div style={{ padding: "32px 16px", textAlign: "center", color: T.textMid, fontSize: 13, lineHeight: 1.6 }}>
                  {t("msg.no_user_found")} "{emailQuery}".
                </div>
              ) : searchResults.length > 0 ? (
                <>
                  <div style={{ padding: "8px 14px 4px", fontSize: 10, fontWeight: 700, color: T.textMid, textTransform: "uppercase", letterSpacing: 0.7 }}>
                    {searchResults.length} {t("msg.result_count")}
                  </div>
          ...
          <>
            ...
            <div style={{ flex: 1, overflowY: "auto" }}>
              {loadingRole ? (
                ...
                <div style={{ padding: "32px 16px", textAlign: "center", color: T.textMid, fontSize: 13 }}>{t("msg.no_contacts")}</div>
              ) : (
