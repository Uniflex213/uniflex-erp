...
function timeAgo(iso: string, now_label: string): string {
  const diff = Date.now() - new Date(iso).getTime();
  const m = Math.floor(diff / 60000);
  if (m < 1) return now_label;
  if (m < 60) return `${m}m`;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: T.bg }}>
      <div style={{ padding: "16px 16px 12px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontSize: 14, fontWeight: 800, color: T.text, letterSpacing: -0.3 }}>{t("msg.messages")}</span>
        <button
          onClick={onNewConversation}
          title={t("msg.new_conversation")}
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {loading ? (
          ...
          <div style={{ padding: "32px 16px", textAlign: "center" }}>
            ...
            <p style={{ margin: 0, fontSize: 13, color: T.textMid, lineHeight: 1.5 }}>{t("msg.no_conversations")}<br/>{t("msg.start_writing")}</p>
          </div>
        ) : (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: isActive ? "rgba(255,255,255,0.65)" : T.textMid, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                    {lastMsg ? (lastMsg.sender_id === currentUserId ? t("msg.you_prefix") : "") + lastMsg.content : (
