...
export default function EmailInboxPanel({ isOpen, onClose, onOpenFullPage, userEmail, onUnreadChange }: Props) {
  ...
  return (
    <>
      <div
        style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 9000, opacity: isOpen ? 1 : 0, pointerEvents: isOpen ? "all" : "none", transition: "opacity 0.25s ease" }}
        onClick={onClose}
      />
      <div
      ...
      {showNewFolder && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 10000, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowNewFolder(false)}>
