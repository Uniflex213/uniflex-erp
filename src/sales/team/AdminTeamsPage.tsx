  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(230,228,224,0.35)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.bgCard, borderRadius: 16, width: "100%", maxWidth: 400, boxShadow: "0 24px 80px rgba(0,0,0,0.2)", textAlign: "center", padding: 32 }}>
        ...
        <h3 style={{ margin: "0 0 8px", fontSize: 16, fontWeight: 800, color: T.text }}>{t("admin_teams.team_approved", "Équipe approuvée !")}</h3>
        <p style={{ fontSize: 13, color: T.textLight, margin: "0 0 20px" }}>
          {t("admin_teams.code_generated_for", "Code généré pour")} <strong>{request.requester_name}</strong> :
        </p>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={copy} style={{ flex: 1, padding: "11px", borderRadius: 10, border: `1.5px solid ${T.main}`, background: copied ? T.main : "#fff", color: copied ? "#fff" : T.main, cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 700, transition: "all 0.2s" }}>
            {copied ? t("admin_teams.copied", "✅ Copié !") : t("admin_teams.copy_code", "Copier le code")}
          </button>
          <button onClick={() => onApprove(code)} style={{ flex: 1, padding: "11px", borderRadius: 10, border: "none", background: T.main, color: "#fff", cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 700 }}>
            {t("admin_teams.confirm", "Confirmer")}
          </button>
        </div>
      </div>
    </div>
...
export default function AdminTeamsPage({ isAdmin }: Props) {
  ...
  const handleDissolve = async (teamId: string) => {
    if (!window.confirm(t("admin_teams.dissolve_confirm", "Dissoudre cette équipe ? Cette action est irréversible."))) return;
  ...
  return (
    <div style={{ padding: "28px 32px", maxWidth: 1200 }}>
      <div style={{ marginBottom: 28 }}>
        <h2 style={{ margin: "0 0 4px", fontSize: 22, fontWeight: 900, color: T.text }}>{t("admin_teams.title", "Gestion des équipes")}</h2>
        <p style={{ margin: 0, fontSize: 13, color: T.textLight }}>{t("admin_teams.subtitle", "Gérez les demandes de création d'équipes et supervisez les équipes actives")}</p>
      </div>

      <div style={{ display: "flex", gap: 14, marginBottom: 28, flexWrap: "wrap" }}>
        {[
          { label: t("admin_teams.active_teams", "Équipes actives"), value: String(activeTeams.length), icon: "👥" },
          { label: t("admin_teams.members_in_teams", "Membres en équipe"), value: String(totalMembers), icon: "👤" },
          { label: t("admin_teams.pending_requests", "Demandes en attente"), value: String(pending.length), icon: "📋" },
      ...
      {pending.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          <h3 style={{ margin: "0 0 14px", fontSize: 14, fontWeight: 800, color: T.text }}>
            {t("admin_teams.pending_requests", "Demandes en attente")} ({pending.length})
          </h3>
          <div style={{ background: T.bgCard, borderRadius: 16, border: `1px solid ${T.border}`, overflow: "hidden" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ background: T.bg }}>
                  {[t("common.date", "Date"), t("admin_teams.requester", "Demandeur"), t("admin_teams.reason", "Raison"), t("admin_teams.estimated_members", "Membres est."), t("admin_teams.region", "Région"), t("common.actions", "Actions")].map(h => (
                    ...
                    <td style={{ padding: "12px 14px" }}>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => setApproving(req)} style={{ padding: "5px 12px", borderRadius: 7, border: "none", background: T.green, color: "#fff", cursor: "pointer", fontFamily: "inherit", fontSize: 11, fontWeight: 700 }}>
                          {t("admin_teams.approve", "Approuver")}
                        </button>
                        {rejectId === req.id ? (
                          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                            <input value={rejectReason} onChange={e => setRejectReason(e.target.value)} placeholder={t("admin_teams.reason_placeholder", "Raison...")} style={{ height: 28, borderRadius: 6, border: `1px solid ${T.border}`, padding: "0 8px", fontSize: 11, fontFamily: "inherit", outline: "none", width: 120 }} />
      ...
      <div style={{ marginBottom: 28 }}>
        <h3 style={{ margin: "0 0 14px", fontSize: 14, fontWeight: 800, color: T.text }}>{t("admin_teams.active_teams", "Équipes actives")} ({activeTeams.length})</h3>
        {activeTeams.length === 0 ? (
          <div style={{ background: T.bgCard, borderRadius: 14, padding: 32, border: `1px solid ${T.border}`, textAlign: "center", color: T.textLight }}>
            {t("admin_teams.no_active_teams", "Aucune équipe active pour le moment")}
          </div>
        ) : (
          <div style={{ background: T.bgCard, borderRadius: 16, border: `1px solid ${T.border}`, overflow: "hidden" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ background: T.bg }}>
                  {[t("admin_teams.team_name", "Nom de l'équipe"), t("admin_teams.code", "Code"), t("admin_teams.members", "Membres"), t("admin_teams.region", "Région"), t("admin_teams.created_on", "Créée le"), t("common.actions", "Actions")].map(h => (
                    <th key={h} style={{ padding: "10px 14px", textAlign: "left", fontSize: 10, fontWeight: 800, color: T.textLight, textTransform: "uppercase", letterSpacing: 0.4, whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
                  ...
                  <tr key={team.id} style={{ borderTop: `1px solid ${T.border}` }}>
                    ...
                    <td style={{ padding: "12px 14px" }}>
                      <button onClick={() => handleDissolve(team.id)} style={{ padding: "5px 10px", borderRadius: 6, border: `1px solid ${T.red}40`, background: T.bgCard, color: T.red, cursor: "pointer", fontFamily: "inherit", fontSize: 11, fontWeight: 600 }}>
                        {t("admin_teams.dissolve", "Dissoudre")}
                      </button>
                    </td>
                  </tr>
                ))}
      ...
      {processed.length > 0 && (
        <div>
          <h3 style={{ margin: "0 0 14px", fontSize: 14, fontWeight: 800, color: T.text }}>{t("admin_teams.processed_requests", "Demandes traitées")}</h3>
          <div style={{ background: T.bgCard, borderRadius: 16, border: `1px solid ${T.border}`, overflow: "hidden" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ background: T.bg }}>
                  {[t("common.date", "Date"), t("admin_teams.requester", "Demandeur"), t("admin_teams.region", "Région"), t("common.status", "Statut"), t("admin_teams.generated_code", "Code généré")].map(h => (
                  ...
                  <tr key={req.id} style={{ borderTop: `1px solid ${T.border}` }}>
                    ...
                    <td style={{ padding: "10px 14px" }}>
                      <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 6,
                        ...
                        {req.status === "approved" ? t("admin_teams.approved", "Approuvée") : t("admin_teams.rejected_status", "Rejetée")}
                      </span>
                    </td>
                    <td style={{ padding: "10px 14px" }}>
                      {req.generated_code ? (
