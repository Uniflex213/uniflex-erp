  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        <div>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#d4a017", textTransform: "uppercase", letterSpacing: 0.5 }}>📦 {t("ws.samples.title", "Mes Samples")}</div>
        </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
        <div style={{ flex: 1, background: "rgba(245,158,11,0.08)", borderRadius: 8, padding: "8px 10px" }}>
          <div style={{ fontSize: 10, color: T.textLight, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 }}>{t("ws.samples.pending", "En attente")}</div>
        ...
        <div style={{ flex: 1, background: "rgba(59,130,246,0.08)", borderRadius: 8, padding: "8px 10px" }}>
          <div style={{ fontSize: 10, color: T.textLight, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 }}>{t("ws.samples.sent", "Envoyés")}</div>
        ...
        <div style={{ flex: 1, background: followUpRequired > 0 ? "rgba(239,68,68,0.08)" : "rgba(34,197,94,0.08)", borderRadius: 8, padding: "8px 10px" }}>
          <div style={{ fontSize: 10, color: T.textLight, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 }}>{t("ws.samples.follow_up", "Follow-up")}</div>
      ...
      {active.length > 0 ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          ...
          {active.length > 3 && (
            <div style={{ fontSize: 11, color: T.textLight, textAlign: "center", paddingTop: 4 }}>+{active.length - 3} {t("ws.samples.others", "autres")}</div>
          )}
        </div>
      ) : (
        <div style={{ fontSize: 12, color: T.textLight, textAlign: "center", padding: "12px 0" }}>{t("ws.samples.none", "Aucun sample actif")}</div>
      )}
    </div>
  );
}
